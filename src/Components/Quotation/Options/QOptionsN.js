...
function QOptionsN() {
  ...
  const [activeStep, setActiveStep] = useContext(QuotationContext);
  const [fromAddress, setFromAddress] = useContext(fromAddressContext);

  useEffect(() => {
    if (Object.keys(fromAddress).length === 0) {
      window.alert("Please enter the Address First");
      setActiveStep(1);
    }
  });
  ...
  const handleBox = e => {
    ...
    if (boxTypeToSend === "hamnipost-box" || boxTypeToSend === "carrier") {
      ...
      window.alert("Please choose the box type first");
    }
  };
  return (
